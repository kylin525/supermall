    ...
    <div class="indicator">
      <slot name="indicator" v-if="showIndicator && slideCount>1">
        <div v-for="(item, index) in slideCount" class="indi-item" :class="{active:index === currentIndex-1}"  :key="index"
          @click="indiItemClick($event,index)"></div>
      </slot>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    data(){
      return {
        ...
        swiperStyle:{},         //得到swiper样式
        totalWidth:0,           //得到swiper宽度
    ...
    methods:{
      ...
      handleDom(){
        ...
        //4.让swiper元素显示第一个(目前是显示前面添加最一个元素)
      ...
      setTransform(position){
         ...
         this.swiperStyle['-wekit-transform']=`translate3d(`+position+`px),0,0`;
